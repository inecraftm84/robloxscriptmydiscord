...
Notify("Welcome to SUPER troll gui NO FE!", "Have fun!", 3)
-- Main frameSUPER
...
titleLabel.BackgroundTransparency = 1
titleLabel.Text           = "SUPER troll gui NO FE"
titleLabel.Font           = Enum.Font.SourceSansBold
titleLabel.TextSize       = 18
titleLabel.TextColor3     = Color3.new(1,1,1)
titleLabel.TextXAlignment = Enum.TextXAlignment.Left
...
-- Initialize
updateGrid()
FunctionManager:register("randompickbodycolor", function()
	while true do
...
end, "Troll", "Colors flicker like disco lights")

FunctionManager:register("RainbowColor", function()
    local TweenService = game:GetService("TweenService")
    while true do
        for _, player in pairs(game.Players:GetPlayers()) do
...
end, "Troll", "Rainbow color effect")

FunctionManager:register("RotateBodyParts", function()
    while true do
        for _, player in pairs(game.Players:GetPlayers()) do
            local char = player.Character
            if char then
...
end, "Troll", "Slow rotate body parts")


FunctionManager:register("FloatingBADWORD", function()
	local badword = {"bitch", "shit", "fuck", "piss", "ass"}
	local playerBillboards = {} -- 儲存每個玩家的BillboardGui
	...
	while true do
		for _, player in pairs(game.Players:GetPlayers()) do
			...
			if char and char:FindFirstChild("Head") then
				...
				-- 更新文字和顏色
				local label = playerBillboards[player]:FindFirstChild("EmojiLabel")
				if label then
					label.Text = badword[math.random(1, #badword)]
...
end, "Troll", "Floating BADWORD above heads")
...
FunctionManager:register("PoopDecalFly", function()
    local poopDecalId = 7437025879 -- 替換為你的貼圖 ID

    while true do
        for i = 1, 15 do
            local part = Instance.new("Part")
            part.Size = Vector3.new(2, 2, 0.5) -- 保持有厚度
            part.Anchored = false
            ...
            game.Debris:AddItem(part, 8) -- 自動清理
        end
        wait(0.3)
    end
...
FunctionManager:register("JOHN DOE", function()
	...
	local redSkyboxAssetId = "rbxassetid://100569691902428"
	...
	if not backgroundSound then
		...
		backgroundSound.SoundId = "rbxassetid://9043345732"
	...
	local function setupCharacter(character)
		...
		tool.Activated:Connect(function()
			...
			s.SoundId = "rbxassetid://9043345732"
			s:Play()

			task.wait(0.2)

			local s2 = Instance.new("Sound", torso)
			s2.SoundId = "rbxassetid://9043345732"
			s2.Volume = 0.2
			s2:Play()
		end)
